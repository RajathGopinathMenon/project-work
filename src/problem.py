...
class GoldProblem:
    def __init__(self, num_cities, *, alpha=0.1, beta=1.5, density=0.1, seed=42):
        ...
        for c in range(1, num_cities):
            self.graph.add_node(c, pos=(cities[c, 0], cities[c, 1]), gold=(1 + 999 * rng.random()))
        ...
        self.shortest_distances = dict(nx.all_pairs_dijkstra_path_length(self.graph, weight='dist'))

    def cost(self, path, weight):
        """Calculates cost for a path segment with given gold weight
        path can be either [u, v] for direct segment or a full path"""
        if len(path) == 2:
    ...
    def baseline(self):
        ...
        total_cost = 0
        for dest, path in nx.single_source_dijkstra_path(self.graph, source=0, weight='dist').items():
            cost = 0
            ...
            for c1, c2 in zip(path, path[1:]):
                cost += self.cost([c1, c2], self.graph.nodes[dest]['gold'])
            total_cost += cost
        return total_cost